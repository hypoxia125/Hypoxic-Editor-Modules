...
if (_modulelogic getVariable ["persistent", false]) then {
	...
	[_modulelogic] spawn {
		...
		while {true} do {
			...
			sleep 2;
			if REPAIR_DEBUG then { systemChat format ["Current Units = %1", _units]};
			if REPAIR_DEBUG then { systemChat format ["Current Classes = %1", _classes]};
		};
	};
};
...
[_modulelogic] spawn {
	...
	while {true} do {
		...
		sleep 1;
		if REPAIR_DEBUG then { systemChat format ["Units Repaired = %1", _units]};
	};
};
